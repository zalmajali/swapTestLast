...
@Component({
  selector: 'app-rechargeoperation',
  templateUrl: './rechargeoperation.page.html',
  styleUrls: ['./rechargeoperation.page.scss'],
})
export class RechargeoperationPage implements OnInit {
  public fullName:any;
  public userId:any;
  ...
  public number:any;
  public chargeOperationResult:any=0;
...
//langSeting
  ...
  public floatD:any;
  public floatTow:any;
  public boxshowOne:any;
  public boxshowTow:any;
  public arrowSide:any;
  public mony:any;
  public currentBalance:any;
  public errorCardNum:any;
  public checkCard:any;
  public chargeSuccess:any;
  public cardNumber:any;
  public enterCardError:any;
  public recharge:any;
  public cardLocations:any;
  public back:any;
  public needHelp:any;
  public callSupport:any;
  ...
  initialiseTranslation(){
    ...
    this.translate.get('boxshowOne').subscribe((res: string) => {
      this.boxshowOne = res;
    });
    this.translate.get('boxshowTow').subscribe((res: string) => {
      this.boxshowTow = res;
    });
    ...
    this.translate.get('checkCard').subscribe((res: string) => {
      this.checkCard = res;
    });
    this.translate.get('chargeSuccess').subscribe((res: string) => {
      this.chargeSuccess = res;
    });
    this.translate.get('cardNumber').subscribe((res: string) => {
      this.cardNumber = res;
    });
    this.translate.get('enterCardError').subscribe((res: string) => {
      this.enterCardError = res;
    });
    this.translate.get('recharge').subscribe((res: string) => {
      this.recharge = res;
    });
    this.translate.get('cardLocations').subscribe((res: string) => {
      this.cardLocations = res;
    });
    this.translate.get('back').subscribe((res: string) => {
      this.back = res;
    });
    this.translate.get('errorCardNum').subscribe((res: string) => {
      this.errorCardNum = res;
    });
    this.translate.get('currentBalance').subscribe((res: string) => {
      this.currentBalance = res;
    });
    this.translate.get('mony').subscribe((res: string) => {
      this.mony = res;
    });
    this.translate.get('needHelp').subscribe((res: string) => {
      this.needHelp = res;
    });
    this.translate.get('callSupport').subscribe((res: string) => {
      this.callSupport = res;
    });
  ...
  async chargeCard(){
    ...
    if(this.number != 0){
      ...
      await loading.present();
      this.storesService.chargeCard(this.userId,this.number).then(async data=>{
        this.returnData = data;
        this.operationResult = this.returnData.Error.ErrorCode;
        if(this.operationResult==1){
          this.chargeOperationResult = 1;
          await this.functionGetData(this.userId);
        }else{
          this.chargeOperationResult = 2;
        }
      }).catch(async e=>{
        this.chargeOperationResult = 2;
      })
  ...
  getCardLocations(){
    this.navCtrl.navigateRoot("/cardlocations");
  }
  chargeinformation(){
